...
const ChildManage = () => {
    ...
    let columns = [
        ...
        {
            title: '拥有角色',
            render: (text, record) => <div>{record?.role?.name || '-'}</div>,
            ellipsis: true,
            hideInSearch: true,
        },
        {
            ...
            dataIndex: 'state',
            hideInSearch: true,
            render: (text, record) => <Switch checked={Boolean(record.state)}
                                              onChange={() => handleSwitchChange(record)} checkedChildren="启用"
