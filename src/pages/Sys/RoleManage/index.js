...
const RoleManage = () => {
    ...
    let columns = [
        ...
        },
        {
            ...
            hideInSearch: true,
            // width: 200,
            renderText: (text, record) => record.routes.map(item=>item.name).join(',')
            // render: (text, record) => <div>{record.routes.map(item=>item.name).join(',')}</div>
        },
        {
            title: '描述',
        ...
        },
        {
            ...
            render: (_, record) => (
                <Space>
                    ...
                    <Popconfirm title="确定删除?" onConfirm={() => handleDeleteRecord(record)} okText="确定"
                                cancelText="取消">
    ...
    const handleUpdate = async fields => {
        console.log(fields)
        const moduleIds = fields.modulesIds.map(arr => {
        ...
        const res = await dispatch({
            ...
            payload: {
                ...
                modulesIds: Array.from(new Set(moduleIds)).join(',')
            }
        })
        hide();
        if (res?.code == 200) {
